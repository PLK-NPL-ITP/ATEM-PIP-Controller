import sys, os
from typing import Optional, TypedDict, Dict, Callable, Union
import collections
...
class ATEMSoftwareController:
    ...
    def __init__(self):
        ...
        # Run the initial check for status in ATEM Control Software in Asynchronous way
        QtCore.QTimer.singleShot(0, self._async_initial_check)

    def _async_initial_check(self):
        """Perform initial PIP source check asynchronously."""
        ...
        self.update_WidgetsStats()

    # ================================================================================
    # FRONTEND UPDATE METHODS
    # ================================================================================
    def update_WidgetsStats(self):
        self.image_widgets["ADV_BKGD_BTN"].set_state("YELLOW")
        
        if self.KEY_ON_AIR:
            self.image_widgets["PIP_SOURCE1_BTN"].set_state('RED' if self.PIP_Source == 1 else 'WHITE')
            self.image_widgets["PIP_SOURCE2_BTN"].set_state('RED' if self.PIP_Source == 2 else 'WHITE')
            self.image_widgets["PIP_SOURCE3_BTN"].set_state('RED' if self.PIP_Source == 3 else 'WHITE')
            self.image_widgets["PIP_SOURCE4_BTN"].set_state('RED' if self.PIP_Source == 4 else 'WHITE')
    ...
    def check_ADV_KEY(self) -> bool:
        try:
            self.next_transition = pag.locateOnScreen("./detect resources/Next Transition.png", grayscale=False, confidence=0.7)
        ...
        try:
            if pag.locateOnScreen("./detect resources/ADV_KEY1_ONAIR_ON.png", region=self.next_transition, grayscale=True, confidence=0.8):
                self.KEY_ON_AIR = True
        except pag.ImageNotFoundException:
            self.KEY_ON_AIR = False
        try:
            if pag.locateOnScreen("./detect resources/ADV_KEY1_PREV_ON.png", region=self.next_transition, grayscale=True, confidence=0.8):
...
# ================================================================================
class ATEMPIPController(ShortcutManager, MainWindow, SignalsHandler, MouseInteractionMixin, ATEMSoftwareController):
    """
    ...
    def __init__(self):
        ...
        # SignalsHandler must be initialized first as MainWindow.init_ui() needs attributeFuncs
        SignalsHandler.__init__(self)
        MainWindow.__init__(self)
        MouseInteractionMixin.__init__(self) 
        ATEMSoftwareController.__init__(self)
